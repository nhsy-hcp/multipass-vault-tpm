...
log_ok "${vault_ver}"
# The staged copy has done its job. Leaving it would let a later run install a
# binary the host has since rolled back from, and it is 550 MB.
rm -f "${VAULT_STAGED}"
...
log_ok "lab tree under ${VM_DIR}"

# Artefacts of the earlier cert-auth design: the PKI directory (TSS2 key blob,
# CSR, certificate) and the hand-rolled login helper. Meaningless now, and
# confusing during the non-exportability demo.
if [[ -e "${VM_DIR}/pki" || -e "${VM_DIR}/tpm-login-request.sh" ]]; then
  log_detected "cert-auth artefacts from an earlier build" "removing ${VM_DIR}/pki and the login helper"
  rm -rf "${VM_DIR}/pki" "${VM_DIR}/tpm-login-request.sh"
fi
...
if [[ -s "${LICENCE_STAGED}" ]]; then
  if [[ -f "${LICENCE_INSTALLED}" ]] && cmp -s "${LICENCE_STAGED}" "${LICENCE_INSTALLED}"; then
    ...
    install -m 0600 -o "${VM_USER}" -g "${VM_USER}" "${LICENCE_STAGED}" "${LICENCE_INSTALLED}"
...
if write_if_changed "${VM_DIR}/env.sh" <<EOF
...
EOF
then
  log_info "Wrote ${VM_DIR}/env.sh"
else
  log_detected "unchanged ${VM_DIR}/env.sh" "leaving it alone"
fi
# write_if_changed installs as root; the lab user must be able to source it.
chown "${VM_USER}:${VM_USER}" "${VM_DIR}/env.sh"
chmod 0644 "${VM_DIR}/env.sh"
log_ok "${VM_DIR}/env.sh owned by ${VM_USER}, mode 0644"
